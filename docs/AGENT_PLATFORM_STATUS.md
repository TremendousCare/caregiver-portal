...
**Phase 1.5 follow-up — Agent loop closure** *(in progress — PR #317 shipped 2026-05-12; PR 2 next)*
**Phase 1.6 — Call Intelligence** *(planned, scoped 2026-05-14, gated on 1.5 follow-up PR 2 baked)*
...
**Next (PR 2 — same pattern, 5 more wire points):** instrument `email compose` (`EmailComposeForm`), `schedule create` (caregiver page), `phase change` (caregiver page), `task complete` (board / caregiver page), `note add` (caregiver page). Each call site is one line invoking `closePendingSuggestionForAction({ entityType, entityId, actionType, params })` after the underlying write succeeds. No new schema, no new edge function — only call sites + tests per surface.

**UI strategy review (deferred until after PR 2 + 2-3 day bake):** Owner has indicated alignment with **pattern C+D** for the long-term destination (outcomes-based dashboard + background-autonomous agent with daily digest), supersedes the current "AI Priorities feed with action buttons" pattern. The discussion happens after PR 2 + bake so it's data-driven — the bake produces real numbers on how often the loop closes organically, which informs whether the current panel needs inline-grading augmentation or can be replaced wholesale by an outcomes view. Phase 2 (recruiting funnel orchestrator) starts with the right UI direction baked in.
...
| **1.5 follow-up** | **Agent loop closure (passive linker)** | **In progress** | PR #317 (2026-05-12) shipped SMS path; PR 2 next | Discovery: AI Priorities dashboard buttons today only navigate — no `agent_actions` rows ever land, autonomy v2 starves of positive signal. PR #317 ships `closePendingSuggestion` shared helper, `close-pending-suggestion` edge function, frontend wrapper, and SMS-compose wire-up. PR 2 adds email / schedule / phase change / task complete / note add. After PR 2 bakes 2-3 days, owner-led UI strategy review (pattern A→C+D direction). |
