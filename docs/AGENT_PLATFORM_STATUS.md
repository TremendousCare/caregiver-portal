...
**Pipeline Health UI** *(in progress 2026-05-16 — UI-A implementation, retires AIPrioritiesPanel + NotificationCenter)*
**Phase 1.6.1 — call_taxonomy + context_memory.related_entity_id + Settings editor** *(shipped 2026-05-16, PR #354)*
**Phase 1.6.2 — call_analyst agent manifest + post-call-processor extension** *(shipped 2026-05-16, PR #355 — 14-day shadow bake gates promotion once owner flips kill_switch OFF)*
...
| **Pipeline Health UI** | **Pattern C+D — daily-driver pipeline view, retires AI Priorities feed** | **In progress** | — | New admin route `/pipeline-health` with phase-grouped table, days-in-phase stalled signal (amber 5d / red 14d), days-since-activity secondary signal, AI suggestions as informational inline badges with click-to-expand reasoning. Three filters (phase multi-select / stalled-only / has-AI-suggestion). Retires `AIPrioritiesPanel` widget + unmounted `NotificationCenter` dead code. Pure UI — no new schema, no new edge function. Digest view `/agent-activity` deferred as UI-B until Phase 2 produces volume worth digesting. Implementation spec: `docs/AGENT_PLATFORM_PIPELINE_HEALTH_SPEC.md`. All five §8 decisions locked 2026-05-15 (PR #350). |
