...
**Pipeline Health UI** *(shipped 2026-05-16, PR #357 — retires AIPrioritiesPanel + NotificationCenter)*
**Phase 1.6.1 — call_taxonomy + context_memory.related_entity_id + Settings editor** *(shipped 2026-05-16, PR #354)*
**Phase 1.6.2 — call_analyst agent manifest + post-call-processor extension** *(shipped 2026-05-16, PR #355 + migration hotfixes #358 + #359; 14-day shadow bake gates promotion once owner flips kill_switch OFF)*
**Phase 1.6.3–1.6.4 — Profile surfaces + cross-domain context recipes** *(planned, sequenced after 1.6.2 shadow bake completes)*
...
| **Pipeline Health UI** | **Pattern C+D — daily-driver pipeline view, retires AI Priorities feed** | **Shipped + closed** | 2026-05-16 (PR #357) | New admin route `/pipeline-health` with phase-grouped table, days-in-phase stalled signal (amber 5d / red 14d), days-since-activity secondary signal, AI suggestions as informational inline badges with click-to-expand reasoning. Three filters (phase multi-select / stalled-only / has-AI-suggestion). Retired `AIPrioritiesPanel` widget + unmounted `NotificationCenter` dead code in the same PR. Pure UI — no new schema, no new edge function. Digest view `/agent-activity` deferred as UI-B until Phase 2 produces volume worth digesting. Implementation spec: `docs/AGENT_PLATFORM_PIPELINE_HEALTH_SPEC.md`; all five §8 decisions locked 2026-05-15 (PR #350). |
| **1.6.1** | **call_taxonomy table + context_memory.related_entity_id additive + Settings editor** | **Shipped + closed** | 2026-05-16 (PR #354) | New `call_taxonomy` table (axis = call_type \| red_flag), `upsert_call_taxonomy_row_v1` SECURITY DEFINER RPC (admin-gated, JWT org-scoped UPSERT on `(org_id, axis, slug)`), 7 call_types + 8 red_flags seeded for Tremendous Care, additive `context_memory.related_entity_type/_id` columns + two partial indexes (`superseded_by IS NULL AND related_entity_id IS NOT NULL`), admin Settings UI panel with add/edit/archive/restore. Pure additive — nothing reads the new column yet. Tests: 81 new specs across migration + RPC + seed + context_memory + helper files. |
| **1.6.2** | **call_analyst agent + post-call-processor extension** | **Shipped + closed** | 2026-05-16 (PR #355 + hotfixes #358 + #359) | New `extractor` runtime shape + `runExtractorHandler` (one-shot, single `submit_call_analysis` tool, no agent loop, ~$0.01/call on Haiku 4.5). Seeds `call_analyst` agent row + initial `agent_versions` snapshot with `kill_switch=true`, `shadow_mode=true`. Extends `ai_suggestions.source_type` CHECK to include `'call_analyst'` (full 6-value enum after #358 hotfix preserved `event_triggered`). Five reusable callContext read helpers in `_shared/operations/agentRuntime/callContext.ts` (consumed inline by the extractor handler; Phase 1.6.4 will wrap them as a formal `callContext` assembler layer so the chat agent inherits the same primitives). `persistCallAnalysis` write helper writes ai_suggestions rows + agent_actions audit rows (phase='shadow' under shadow_mode) + call_sessions.ai_summary stamp (idempotency anchor). post-call-processor extension invokes the agent after `markTranscriptFetched()` from a **separate pending-analysis pool** (per PR #355 Codex P1 fix — selector is `ai_summary IS NULL AND transcript_fetched_at IS NOT NULL`, retries safely when kill_switch flips OFF). System prompt + structured-output schema in `docs/agent-prompts/call_analyst.md` (voice-agent compatible from day one per Phase 7 reuse). Memory writes deferred to 1.6.3 with the Memories review UI — memory_candidates emitted as a draft array on `call_sessions.ai_outcome` only. Owner flips `kill_switch=false` via Settings to start the 14-day shadow bake. |
...
| 2026-05-15 | #350 | Pipeline Health spec | Locked the UI direction conversation: pattern C+D framing → "Pipeline Health" daily-driver surface that retires the AI Priorities feed. Implementation contract in `docs/AGENT_PLATFORM_PIPELINE_HEALTH_SPEC.md` with all five §8 decisions locked (sidebar placement, default route, stall thresholds, AI badge interaction depth, empty-state behavior). Markdown only. |
| 2026-05-16 | #354 | 1.6.1 | `call_taxonomy` table (axis = call_type \| red_flag), `upsert_call_taxonomy_row_v1` SECURITY DEFINER RPC (admin-gated UPSERT on (org_id, axis, slug)), 7 call_types + 8 red_flags seeded for Tremendous Care, additive `context_memory.related_entity_type/_id` columns + two partial indexes, admin Settings UI panel with add/edit/archive/restore. 81 new specs. **Phase 1.6.1 closed.** |
| 2026-05-16 | #355 | 1.6.2 | call_analyst agent + post-call-processor extension. New `extractor` runtime shape + `runExtractorHandler` (one-shot single-tool, Haiku 4.5). Seeds call_analyst agent row with `kill_switch=true, shadow_mode=true`. Extends `ai_suggestions.source_type` CHECK to include `call_analyst`. Five reusable callContext primitives in `agentRuntime/callContext.ts`. `persistCallAnalysis` write helper (ai_suggestions rows + agent_actions audit with phase='shadow' under shadow_mode + call_sessions stamp last). post-call-processor extension invokes the agent from a separate pending-analysis pool (per Codex P1 #r3251942660: original inline invocation tied analyst eligibility to transcript_fetched_at, permanently excluding kill_switched calls — fixed via two-pool design with `ai_summary IS NULL` selector). System prompt + structured-output schema in `docs/agent-prompts/call_analyst.md` (voice-agent compatible from day one). 66 new specs. |
| 2026-05-16 | #357 | Pipeline Health UI | Implementation of UI-A per #350's spec. New admin route `/pipeline-health` with phase-grouped table, days-in-phase stalled signal (amber 5d / red 14d), days-since-activity secondary signal, AI suggestions as informational inline badges with click-to-expand reasoning. Three filters. Retired `AIPrioritiesPanel` widget + unmounted `NotificationCenter` dead code in the same PR. 30 new specs in `pipelineHealth.test.js`; full suite 4,226 passing. |
| 2026-05-16 | #358 | 1.6.2 hotfix | Migration deploy failure on production — the original 1.6.2 source_type CHECK extension shipped a 5-value enum that dropped `event_triggered`, a value the proactive planner had been writing since 20260321220555. Production rows violated the new CHECK and the whole transaction rolled back. Fix: include all 6 values in the forward migration + add a separate idempotent repair migration (20260516030000) per Codex P2 #r3252045794 to cover the rare case where 020000 was applied successfully with the buggy version. Belt-and-suspenders regression-guard DO block fails the migration if event_triggered is missing. |
| 2026-05-16 | #359 | 1.6.2 hotfix | Migration timestamp collision with `20260516020000_bd_mileage_entries.sql` (PR #356, parallel workstream) — Supabase tracks applied migrations by timestamp prefix, so two files at the same slot would silently skip one another. Renamed the 1.6.2 source_type CHECK migration from `20260516020000` → `20260516023000`. **Phase 1.6.2 closed pending owner deploy + kill_switch flip.** |
