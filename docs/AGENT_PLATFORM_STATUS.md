...
**Phase 1.6.1 — call_taxonomy + context_memory.related_entity_id + Settings editor** *(in progress 2026-05-16 — schema + RPC + seed + admin UI)*
**Phase 1.6.2–1.6.4 — call_analyst manifest, profile surfaces, context-recipe extension** *(planned, sequenced after 1.6.1 ships)*
...
| **1.6.1** | **call_taxonomy table + context_memory.related_entity_id additive + Settings editor** | **In progress** | — | New `call_taxonomy` table (axis = call_type \| red_flag), `upsert_call_taxonomy_row_v1` SECURITY DEFINER RPC (admin-gated, JWT org-scoped UPSERT on `(org_id, axis, slug)`), 7 call_types + 8 red_flags seeded for Tremendous Care, additive `context_memory.related_entity_type/_id` columns + two partial indexes (`superseded_by IS NULL AND related_entity_id IS NOT NULL`), admin Settings UI panel with add/edit/archive/restore. Pure additive — nothing reads the new column yet. Tests: 81 new specs across migration + RPC + seed + context_memory + helper files. |
