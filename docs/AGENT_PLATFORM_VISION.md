...
| **Call Intelligence Agent** | New extractor agent. **Ships in Phase 1.6** | Shared infrastructure, not domain-specific. Runs after every transcribed call (recruiting / intake / scheduling / care). Produces structured output: classified call type + summary + action items + red flags + memory candidates + sentiment. Outputs flow into the shared kernel (`ai_suggestions`, `context_memory`) so existing and future agents consume them at context-assembly time. Outcome: operator accepts the action item within 7 days (operator-confirmed escape clause per Prime Directive #2). Coarse first — one `call_analyst` covers all call types via an editable taxonomy; splits only when data signals. |
| **Intake / Lead Management Agent** | Third new agent | Moves new client leads through inquiry → assessment → start-of-care. Smaller blast radius than scheduling, cleanest outcome signal (`start_of_care_date` set). Ships in Phase 3. |
| **Scheduling Agent** | Fourth new agent | Fills open shifts, handles call-offs, manages caregiver-shift matching, learns availability patterns. Highest stakes, biggest external demo wow-factor — sequenced behind recruiting + intake so the runtime, audit log, and autonomy-v2 algorithm are battle-tested before scheduling exposes its larger blast radius. Outcome: verified clock-in. |
| **Care Coordination Agent** | Fifth new agent | Ongoing client care management, family communication, escalations, visit follow-ups. Heavy memory + cross-agent dependency; ships after the inter-agent dispatch layer (Phase 5). |
...
- **Phase 1.6 inserts Call Intelligence as a shared extractor agent** (locked 2026-05-14, supersedes the earlier "call intelligence lives inside whichever agent owns the entity" framing). Call intelligence is per-org infrastructure, not per-domain — recruiting, intake, scheduling, and care coordination all read from the same `context_memory` and `ai_suggestions` rows the call_analyst writes. Building it inside each domain agent would fragment the work; deferring it to Phase 7 (voice) would starve every intermediate phase of structured call data. Slotting it before Phase 2 ensures the recruiting orchestrator launches with call context already flowing. Schema is voice-agent-compatible from day one so Phase 7's voice agent emits identical shapes and downstream consumers are unchanged.
