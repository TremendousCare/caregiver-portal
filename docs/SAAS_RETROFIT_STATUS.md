...
- ⏳ **B2b in flight (2026-04-30)** — add org-scoped permissive RLS policies (`tenant_isolation_<table>_<cmd>`) alongside the existing permissives on the 40 in-scope B1 tables. Skips `email_accounts` and `email_routing` (RLS enabled, zero policies, service-role-only — a permissive policy there would *open* access). 160 policies total (40 × 4 commands). Strict / fail-closed predicate `org_id = nullif(auth.jwt() ->> 'org_id', '')::uuid`. Pure additive: TC's existing `is_staff()` / `current_user_caregiver_id()` policies still grant in parallel, so no behavior change today; the new policies become the only enforcers when permissives drop in B5.
