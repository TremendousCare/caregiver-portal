...
- Native portal invoicing (replaces "manual QuickBooks invoicing at launch" — see Decisions locked). Portal generates draft invoices weekly from the prior workweek's completed shifts, exposes an approval workflow that mirrors payroll, and exports to QuickBooks via CSV. Stripe / direct QBO API integration is a future PR.
...
- **Billing** (revised 2026-04-30): native invoicing in the portal at launch with QuickBooks CSV export. The portal is the source of truth for "which client gets billed how much for which week" and exposes the same draft → approve → export lifecycle as payroll. QBO remains the system of record for accounts receivable until a future phase adds native A/R, payment tracking, and (optionally) Stripe / direct QBO API integration. Replaces the original "manual QuickBooks invoicing at launch" plan: that decision was locked when invoicing felt like a system-of-record problem; once the team realized the portal already had every data point QBO needed (clients, shifts, billable_rate, hours via clock events), generating invoices in-app and exporting CSV is strictly more useful than re-keying. See `docs/INVOICING.md` for the foundation schema and Phase 1 → 4 plan.
