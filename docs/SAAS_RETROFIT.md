...
- **Drop the Phase B `org_id` defaults.** The `DEFAULT (SELECT id FROM organizations WHERE slug = 'tremendous-care')` clause that protected Tremendous Care during Phases B–D becomes a footgun once multiple paying customers exist (any insert path that forgets `org_id` would silently mis-attribute rows to Tremendous Care). Audit every insert path for explicit `org_id`, then `ALTER TABLE … ALTER COLUMN org_id DROP DEFAULT` on every tenant-sensitive table.
...
- **Phase B `org_id` column default** (locked 2026-04-26): `DEFAULT (SELECT id FROM organizations WHERE slug = 'tremendous-care')`. Subselect-as-default rather than a hardcoded UUID literal — keeps the Tremendous Care id out of 30+ migration files and resilient to any future identity reissue. Performance is negligible (1-row table, unique index on slug, sub-millisecond lookup per insert).
- **Phase B default lifecycle** (locked 2026-04-26): the `org_id` default stays through Phases B, C, and D as a single-tenant safety net so any code path that forgets to pass `org_id` still lands rows in Tremendous Care. **In Phase E** the default is dropped and explicit `org_id` becomes mandatory on every insert path. A Phase E follow-up task tracks this.
