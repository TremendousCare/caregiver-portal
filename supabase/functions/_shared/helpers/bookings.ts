...
// ─── Graph notification payload extraction ───────────────────────────────
// Microsoft sends a JSON body like:
//   { "value": [ { subscriptionId, clientState, changeType,
//                  resource, resourceData: { id, "@odata.type", ... } } ] }
// `parseGraphNotifications` returns one structured entry per item or an
// empty array when the body isn't shaped right. The webhook validates
// each entry's clientState against the stored subscription before
// trusting `resource` enough to fetch back from Graph.
...
export function parseGraphNotifications(
  ...
  for (const raw of arr) {
    ...
    const subscriptionId =
      typeof raw.subscriptionId === "string" ? raw.subscriptionId : "";
    const clientState =
      typeof raw.clientState === "string" ? raw.clientState : "";
    const changeType =
      typeof raw.changeType === "string" ? raw.changeType : "";
    const resource = typeof raw.resource === "string" ? raw.resource : "";
    // Graph sends the appointment ID either in resourceData.id or
    // appended to the resource path. We accept either.
    const resourceData =
      raw.resourceData && typeof raw.resourceData === "object"
        ? raw.resourceData
        : {};
    const idFromData =
      typeof resourceData.id === "string" ? resourceData.id : "";
    const idFromPath = (() => {
      const m = resource.match(/appointments\/([^/?#]+)/);
      return m ? decodeURIComponent(m[1]) : "";
    })();
    const appointmentId = idFromData || idFromPath;
    if (!subscriptionId || !appointmentId) continue;
    out.push({
      subscriptionId,
      clientState,
      changeType,
