...
async function handleSubmitSignature(
  ...
  try {
    ...
    const certPage = certPdf.addPage([612, 792]); // Letter size
    const signerName = cg ? `${cg.first_name} ${cg.last_name}` : "Unknown";
    const signedAt = new Date().toISOString();
    const consentAt = envelope.consent_timestamp || signedAt;

    let y = 720;
    const drawLine = (text: string, size = 11, color = rgb(0, 0, 0)) => {
      certPage.drawText(text, { x: 50, y, size, color });
      y -= size + 6;
    };
    const drawSpacer = (h = 10) => { y -= h; };

    // Header
    certPage.drawText("CERTIFICATE OF COMPLETION", { x: 150, y: 740, size: 18, color: rgb(0.18, 0.31, 0.55) });
    y = 710;
    certPage.drawText("Tremendous Care \u2014 Electronic Signature", { x: 180, y, size: 10, color: rgb(0.45, 0.45, 0.45) });
    y = 680;

    // Divider line
    certPage.drawRectangle({ x: 50, y, width: 512, height: 1, color: rgb(0.8, 0.83, 0.87) });
    y -= 20;

    drawLine("ENVELOPE DETAILS", 13, rgb(0.18, 0.31, 0.55));
    drawSpacer(4);
    drawLine(`Envelope ID: ${envelope.id}`);
    drawLine(`Status: Completed`);
    drawLine(`Sent: ${new Date(envelope.sent_at || envelope.created_at).toLocaleString("en-US")}`);
    drawLine(`Signed: ${new Date(signedAt).toLocaleString("en-US")}`);
    drawSpacer(10);

    drawLine("DOCUMENTS SIGNED", 13, rgb(0.18, 0.31, 0.55));
    drawSpacer(4);
    for (let i = 0; i < templates.length; i++) {
      const tpl = templates[i];
      const hash = documentHashes[i] || "N/A";
      drawLine(`${i + 1}. ${tpl.name}`);
      drawLine(`   SHA-256: ${hash}`, 8, rgb(0.45, 0.45, 0.45));
      drawSpacer(2);
    }
    drawSpacer(10);

    drawLine("SIGNER INFORMATION", 13, rgb(0.18, 0.31, 0.55));
    drawSpacer(4);
    drawLine(`Name: ${signerName}`);
    drawLine(`Email: ${cg?.email || "N/A"}`);
    drawLine(`IP Address: ${ip}`);
    drawLine(`User Agent: ${ua.substring(0, 80)}${ua.length > 80 ? "..." : ""}`, 9);
    drawSpacer(10);

    drawLine("ESIGN ACT CONSENT", 13, rgb(0.18, 0.31, 0.55));
    drawSpacer(4);
    drawLine(`Consent Accepted: ${new Date(consentAt).toLocaleString("en-US")}`);
    drawLine(`Consent IP: ${envelope.consent_ip || ip}`);
    drawLine(`The signer agreed that their electronic signature is the legal`);
    drawLine(`equivalent of their handwritten signature under the ESIGN Act`);
    drawLine(`(15 U.S.C. \u00A7 7001).`);
    drawSpacer(10);

    drawLine("AUDIT TRAIL", 13, rgb(0.18, 0.31, 0.55));
    drawSpacer(4);
    const trail = [...(envelope.audit_trail || []),
      { action: "signed", at: signedAt, ip }];
    for (const entry of trail) {
      const ts = entry.at ? new Date(entry.at).toLocaleString("en-US") : "N/A";
      drawLine(`${ts} \u2014 ${entry.action}${entry.ip ? ` (IP: ${entry.ip})` : ""}`, 9);
    }
    drawSpacer(20);
    ...
    y -= 14;
    certPage.drawText(
      "This document is a record of the electronic signing process. The signed documents are tamper-evident",
      { x: 50, y, size: 7, color: rgb(0.45, 0.45, 0.45) },
    );
    y -= 10;
    certPage.drawText(
      "and verified by SHA-256 cryptographic hash. Generated by Tremendous Care eSign System.",
      { x: 50, y, size: 7, color: rgb(0.45, 0.45, 0.45) },
    );
    ...
    try {
      const certResponse = await fetch(`${SUPABASE_URL}/functions/v1/sharepoint-docs`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,
      ...
      if (!certResult.error) certDocId = certResult.doc_id || null;
    } catch (certErr) {
      console.error("[esign] Certificate upload error:", certErr);
    }
  } catch (certErr) {
    console.error("[esign] Certificate generation error:", certErr);
  }

  // Update envelope status with full compliance data
  ...
  await supabase
    ...
    .update({
      ...
      document_hashes: documentHashes.map((hash: string, i: number) => ({
      ...
      completion_certificate_doc_id: certDocId,
      audit_trail: appendAudit(envelope.audit_trail, "signed", {
        ...
        tasks_completed: completedTasks,
        certificate_generated: !!certDocId,
      }),
    })
    .eq("id", envelope.id);
  ...
  if (envelope.sent_by && envelope.sent_by.includes("@")) {
    try {
      const signerName = cg ? `${cg.first_name} ${cg.last_name}` : "A caregiver";
      const docDesc = templates.length > 1
      ...
      await fetch(`${SUPABASE_URL}/functions/v1/bulk-email`, {
        ...
        body: JSON.stringify({
          to: [envelope.sent_by],
          subject: `eSignature Completed \u2014 ${signerName}`,
          body: `<p><strong>${signerName}</strong> has completed signing ${docDesc}.</p><p>Signed documents have been uploaded to SharePoint. A Certificate of Completion has been generated for your records.</p><p><a href="${PORTAL_URL}" style="display:inline-block;padding:10px 20px;background:#2E4E8D;color:#fff;border-radius:8px;text-decoration:none;font-weight:bold;">View in Portal</a></p><p>Tremendous Care eSign System</p>`,
          sent_by: "esign-system",
        }),
      });
      await supabase.from("esign_envelopes").update({ sender_notified: true }).eq("id", envelope.id);
    } catch (_) { /* fire-and-forget */ }
  }
...
// Persists ESIGN Act consent acceptance with timestamp, IP, and user-agent.
...
async function handleRecordConsent(
  ...
  if (!envelope) return jsonResponse({ error: "Invalid token" }, 404);
  if (envelope.status === "signed") return jsonResponse({ success: true });
  ...
  await supabase
    ...
    .update({
      ...
      audit_trail: appendAudit(envelope.audit_trail, "consent_accepted", { ip, ua }),
    })
    .eq("id", envelope.id);

  return jsonResponse({ success: true, consent_recorded: true });
}

// ─── Action: Decline ───
// Allows signer to formally decline signing and provide a reason.
async function handleDecline(
  ...
  const { data: envelope } = await supabase
    .from("esign_envelopes")
    .select("id, status, audit_trail, caregiver_id, template_names, sent_by")
    .eq("signing_token", token)
    .single();

  if (!envelope) return jsonResponse({ error: "Invalid token" }, 404);
  if (envelope.status === "signed") return jsonResponse({ error: "Cannot decline a signed envelope." }, 400);
  ...
  await supabase
    ...
    .update({
      ...
      audit_trail: appendAudit(envelope.audit_trail, "declined", { ip, ua, reason: reason || "No reason provided" }),
    })
    .eq("id", envelope.id);
  ...
  try {
    const { data: cg } = await supabase
      .from("caregivers")
      .select("first_name, notes")
      .eq("id", envelope.caregiver_id)
      .single();
    if (cg) {
      const docDesc = envelope.template_names?.length > 1
        ? `${envelope.template_names.length} documents`
        : (envelope.template_names?.[0] || "document");
      const note = {
        text: `eSignature declined \u2014 ${docDesc}${reason ? `: "${reason}"` : ""}`,
        type: "esign",
        timestamp: Date.now(),
        author: "eSign System",
        outcome: "signing declined",
        direction: "inbound",
      };
      await supabase
        .from("caregivers")
    ...
    try {
      ...
      await fetch(`${SUPABASE_URL}/functions/v1/bulk-email`, {
        ...
        body: JSON.stringify({
          ...
          body: `<p>${signerName} has declined to sign ${docDesc}.</p>${reason ? `<p><strong>Reason:</strong> ${reason}</p>` : ""}<p>You can resend the signing request from the caregiver's profile page.</p><p>Tremendous Care eSign System</p>`,
          sent_by: "esign-system",
        }),
      });
    } catch (_) { /* fire-and-forget */ }
