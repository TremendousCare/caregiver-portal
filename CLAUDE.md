...
7. **Sandwich every LLM call (tiered by risk)** — Deterministic preflight (validate inputs, enforce rules) → LLM (creative work) → Deterministic verify (check output against source of truth). Never trust raw LLM output for high-stakes actions. **Tier the verification to avoid over-engineering:** Low-risk actions (read-only queries, adding notes) get lightweight validation only — no reflection loop. High-risk actions (SMS sends, bulk messaging, phase changes, anything touching money or compliance) get the full sandwich with a reflection step (additional Claude call to verify-your-work before finalizing). The reflection loop roughly doubles token cost per interaction, so applying it universally is wasteful; applying it selectively to high-stakes actions is the gold standard approach most teams don't mention.
