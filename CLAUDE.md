...
- **pg_cron jobs**: automation-cron (every 30min, job 1), outcome-analyzer (every 4h, job 2), payroll-generate-timesheets (Mondays 13:00 UTC, job 3 — generates draft timesheets for the prior Mon→Sun workweek per Phase 3 of the Paychex integration), poll-bookings-appointments (every 5min — pulls Microsoft Bookings appointments per org and reconciles into `caregiver_interviews`; polling is the only viable path because Graph does not support webhook subscriptions for the bookingAppointment resource), service-plan-extend-ongoing (Mondays 14:00 UTC — for every service_plan flagged `is_ongoing=true` and `status='active'`, materializes any missing shifts so the rolling window stays at ~12 weeks of runway from `now`; idempotent via dedupe + `last_generated_through` bookkeeping), assessment-transcribe-reconcile (every 5min — safety net for the in-home assessment transcription pipeline; re-submits assessments stuck in `uploaded`/`transcribing` whose Deepgram async callback was lost and gives up after `maxAttempts`, since Deepgram's callback is the happy path and this recovers the rest), quickbooks-token-refresh (every 30min — proactively refreshes QuickBooks OAuth access tokens before their 1-hour expiry, rotates the 100-day refresh token in lockstep, flips connections to `status='reauth_required'` when Intuit returns 401/invalid_grant so the Settings card surfaces the prompt)
