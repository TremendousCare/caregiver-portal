...
function SectionCard({ section, data, tasks, onEdit, editDisabled }) {
  ...
  // Task-based sections (ADL / IADL) carry BOTH structured field data
  // (in version.data) and a shift-task list. Either one counts as
  // "entered" so the section renders its content inline rather than
  // hiding behind an "Add" button.
  const hasTasks = Array.isArray(tasks) && tasks.length > 0;
  const hasData = usesTasks
    ? sectionHasFieldContent(section, data) || hasTasks
      ...
      {orderedCategories.map((category) => {
        ...
        return (
          <div key={category} className={s.taskGroup}>
            ...
            <TaskItems tasks={groups[category]} />
          </div>
        );
      })}
    </div>
...
// Flat list of task rows (no category heading — used when the caller
// already provides the grouping label, e.g. the per-group ADL/IADL view).
...
// ─── Grouped fields + tasks (ADL / IADL sections) ────────────
// Renders the structured field data AND the shift-task list inline,
// organized by the section's `groups` metadata (Ambulation, Bathing,
// …). Each group shows its populated fields followed by the tasks
// filed under its category. Groups with neither are skipped so the
// view stays tight. Any tasks whose category isn't covered by a group
// (legacy data) fall through to a trailing "Other" block so nothing
// is ever hidden.

function SectionFieldsAndTasks({ section, data, tasks }) {
  // Defensive: a task section without group metadata still renders
  // its raw fields + the standard category-grouped task list.
  ...
  const groupCategories = new Set(
    section.groups.map((g) => g.taskCategory).filter(Boolean),
  );
  const orphanTasks = (tasks || []).filter((t) => !groupCategories.has(t.category));

  return (
    <div className={s.taskGroups}>
      {section.groups.map((group) => {
        const fields = getFieldsForGroup(section, group.id)
          .filter((f) => !isEmptyValue(data?.[f.id]));
        const groupTasks = tasksByCategory[group.taskCategory] || [];
        if (fields.length === 0 && groupTasks.length === 0) return null;

        return (
          <div key={group.id} className={s.taskGroup}>
            <div className={s.taskGroupLabel}>{group.label}</div>
            {fields.length > 0 && (
              <dl className={s.fieldList}>
                {fields.map((field) => (
                  <div key={field.id} className={s.fieldRow}>
                    <dt className={s.fieldLabel}>{field.label}</dt>
                    <dd className={s.fieldValue}>
                      {formatCarePlanFieldValue(field, data[field.id])}
                    </dd>
                  </div>
                ))}
              </dl>
            )}
            {groupTasks.length > 0 && <TaskItems tasks={groupTasks} />}
          </div>
        );
      })}
      ...
      {orphanTasks.length > 0 && (
        <div className={s.taskGroup}>
          <div className={s.taskGroupLabel}>Other</div>
          <TaskItems tasks={orphanTasks} />
        </div>
      )}
    </div>
  );
