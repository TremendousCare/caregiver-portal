...
export function ServicePlansPanel({ client, currentUser, showToast }) {
  ...
  const handleStatusShortcut = async (plan, nextStatus) => {
    // Ending a plan also tears down its upcoming, un-worked shifts so the
    // calendar isn't left littered with orphaned shifts from a plan that's
    // no longer in effect. Completed / in-progress / past shifts are kept.
    if (nextStatus === 'ended') {
      const ok =
        typeof window === 'undefined' ||
        window.confirm(
          'End this service plan?\n\nUpcoming, unworked shifts generated by this plan ' +
            'will be removed from the calendar. Past, completed, and in-progress shifts ' +
            'are kept.',
        );
      if (!ok) return;
      try {
        // Stop the ongoing-extension cron from re-materializing shifts,
        // and mark the plan ended, before clearing the calendar.
        await updateServicePlan(plan.id, { status: 'ended', isOngoing: false });
        const removed = await deleteFutureShiftsForPlan(plan.id);
        showToast?.(
          removed > 0
            ? `Plan ended — removed ${removed} upcoming shift${removed === 1 ? '' : 's'}`
            : 'Plan ended',
        );
        await loadPlans();
      } catch (e) {
        console.error('End plan failed:', e);
