...
/**
 * Bulk-insert many shifts in a single request.
 *
 * Unlike createShift, this does NOT fire shift automations. It's used
 * by the Generate Shifts dialog to materialize a recurring series:
 * those shifts inherit their caregiver from the plan's standing
 * "regular caregiver" rules, which is a schedule the office already
 * committed to — firing a per-shift `shift_assigned` SMS for each of
 * (potentially) dozens of generated shifts would spam the caregiver.
 * This mirrors the service-plan-extend-ongoing cron, which also inserts
 * directly without dispatching automations.
 *
 * Returns the inserted shifts (mapped to app shape).
 */
...
// Future shifts in any of these statuses are "not yet worked" and are
// safe to remove when a plan is ended. Worked / historical states
// (completed, in_progress, no_show, cancelled) are preserved so payroll
// and clock-in history are never destroyed.
export const DELETABLE_FUTURE_SHIFT_STATUSES = ['open', 'offered', 'assigned', 'confirmed'];

/**
 * Delete the future, un-worked shifts generated by a service plan.
 *
 * Used when a plan is ended so its upcoming shifts stop cluttering the
 * calendar. Scoped deliberately tight:
 *   - `start_time >= asOf` — only future shifts; the past is history.
 *   - status in DELETABLE_FUTURE_SHIFT_STATUSES — never touches
 *     completed / in_progress / no_show / cancelled rows.
 *
 * Returns the number of shifts removed.
 */
export const deleteFutureShiftsForPlan = async (servicePlanId, { asOf } = {}) => {
  if (!isSupabaseConfigured()) return 0;
  if (!servicePlanId) return 0;
  ...
  const { data, error } = await supabase
    ...
    .in('status', DELETABLE_FUTURE_SHIFT_STATUSES)
    .select('id');
  if (error) throw error;
  return (data || []).length;
};
