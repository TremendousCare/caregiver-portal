...
export const getPhaseProgress = (caregiver, phaseId) => {
  ...
  const done = tasks.filter((t) => caregiver.tasks?.[t.id]).length;
...
export const getOverallProgress = (caregiver) => {
  ...
  const done = allTasks.filter((t) => caregiver.tasks?.[t.id]).length;
...
export const isGreenLight = (caregiver) => {
  ...
  return required.every((t) => caregiver.tasks?.[t]);
};
