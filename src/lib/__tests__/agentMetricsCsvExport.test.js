...
describe('buildAgentActionsCsv', () => {
  it('produces a header line + one row per action', () => {
    const csv = buildAgentActionsCsv([row(), row({ id: 'a2', chain_seq: 8, outcome_id: null })], []);
  ...
  it('joins outcome data when available', () => {
    const csv = buildAgentActionsCsv(
      [row()],
      [{ id: 'o1', outcome_type: 'response_received', resolved_at: '2026-05-12T11:00:00Z' }],
    );
  ...
  it('writes empty cells when outcome missing', () => {
    const csv = buildAgentActionsCsv([row({ outcome_id: null })], []);
    const dataLine = csv.split('\n')[1];
